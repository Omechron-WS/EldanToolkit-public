using EldanToolkit.WildStar.TestBed;
using EldanToolkit.WildStar.TestBed.TextTable.IO;
...
namespace EldanToolkit.WildStar.TestBed.TextTable
{
	public static class TextTableLoader
	{
		public static DataTable Load(string path)
		{
			...
			using (var reader = new BinaryReader(stream, Encoding.Unicode))
			{
				...
				table.SetUserData("Code", reader.ReadWideString((int)header.CodeLength));

				stream.Position = header.DescriptionOffset + headerSize;
				table.SetUserData("Description", reader.ReadWideString((int)header.DescriptionLength));

				table.SetUserData("Language", header.Language);

				table.SetUserData("Unknown1", header.Unknown1);
